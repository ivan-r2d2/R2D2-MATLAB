% Demo_62: Analize sampling and density evaluation times (M) - PAPER
%  Author: Ivan A. Calle Flores
%   NOTES: There is a high variability in the times
...
u = [v; w];
%  4.2. COMPUTE AND PLOT THE FINAL "noise-free" POSE
xp = ideal_robot.noise_free_motion_model_velocity(u, dt);
    ...
    for n=1:REP
        switch(MOTION_MODEL)
            case {'STANDARD','STANDARD2', 'IMPROVED'}
                % Sampling
                T1 = tic;
                Xp = sample_motion_model_velocity(X, u, dt, alpha,...
                        MOTION_MODEL);
                t = toc(T1)*1000;
                T_SAMPLING(n) = t;
                % Density p(x'|x,u)
                T1 = tic;
                PM = motion_model_velocity(Xp, u, x, dt, alpha,...
                        MOTION_MODEL);
                t = toc(T1)*1000;
                T_DENSITY(n) = t;
            ...
            case 'GAUSSIAN'
                % Sampling
                T1 = tic;
                [mu, R] = my_robot.compute_gaussian_model_velocity(u,dt);
                Xp      = mvnrnd(mu,R,M(i));
                Xp(:,3) = pi_to_pi(Xp(:,3));
                t = toc(T1)*1000;
                T_SAMPLING(n) = t;
...
plot(ellipse_points(1,:),ellipse_points(2,:),'y','LineWidth',2);  
