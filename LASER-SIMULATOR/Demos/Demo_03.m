% Demo_03: Simulation using selected beams
...
K = 500;         % Number of beams (The hokuyo has 513 in [-pi/2, pi/2])
my_laser.set_mode_operation('NORMAL',K);
my_laser.set_theta_selected_index(10);  % Select number of rays
%  4.2. SET PARAMETERS
...
handle_laser_full = zeros(1,K+1);
for f=1:K
    handle_laser_full(1,f) = line(0,0,'Color','m');
end
handle_laser_full(K+1) = plot(0,0,'+k', 'MarkerSize', 5, 'LineWidth',2);
% To plot the selected beams
handle_laser_selected = zeros(1,my_laser.K_SELECTED+1);
for f=1:my_laser.K_SELECTED
    handle_laser_selected(1,f) = line(0,0,'Color','g');
end
handle_laser_selected(my_laser.K_SELECTED+1) = plot(0,0,'+k',...
...
my_laser.display_laser(x, s_full, handle_laser_full)
my_laser.display_laser(x, s, handle_laser_selected)
my_laser.display_laser_end_points(s)
...
for i = 1:N
    ...
    %  Plot results
    my_laser.display_laser(xp, s_full, handle_laser_full)
    my_laser.display_laser(xp, s, handle_laser_selected)
    my_laser.display_laser_end_points(s)
end